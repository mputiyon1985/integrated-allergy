...
export default function Loading() {
  return (
    <>
      <TopBar
        title="Nurses"
        breadcrumbs={[{ label: 'Integrated Allergy IMS' }, { label: 'Nurses' }]}
        actions={<div className="btn btn-primary" style={{ opacity: 0.4, pointerEvents: 'none' }}>+ Add Nurse</div>}
      />
            ...
            <tbody>
              {Array.from({ length: 3 }).map((_, i) => (
                <tr key={i} style={{ borderBottom: '1px solid #f3f4f6' }}>
                  ...
                  {[160, 50, 160, 140, 110, 90, 60, 120].map((w, j) => (
